...
export class ItemEditPage extends BasePage {
      ...
      this.checkPermission().then((result) => {
        if (result) {
          ...
          BarcodeScanner.startScan()
                ...
                this.barcodeService.getBarCodeData(barcode.content).subscribe(
                  ...
                  async (err) => {
                    ...
                    const prompt = await this.alertController.create({
                      ...
                      message:
                        'Upc Code was not found, Do you want to take a picture?',
                      buttons: [
                        {
                          text: 'No',
                          handler: () => {
            ...
            .catch((err) => {
              this.uxNotifierService.showToast(
                'There was an error scanning the barcode/qr code!',
                this._constants.ToastColorBad
              );
            });
        }
        ...
        if (result && result.files) {
          ...
          if (file) {
            if (file.mimeType == 'application/pdf') {
              ...
              this.TempActiveItem.Image = `data:${file.mimeType};base64,${file.data}`;
            }
  ...
  private async saveToFirebase(file: any): Promise<UploadTaskSnapshot> {
    ...
    if (this._type === 'file') {
      await this.firebaseService
        .uploadOtherFiles(file, fileName, this.User.Id)
        .then(
          (x) => {
        ...
        if (selectedProfileItem.IsSaveToRoom) {
          ...
          if (this._type == 'Bookmark') {
            ...
            selectedProfileItem.Lineitems.forEach(async (x) => {
              if (x.IsChecked) {
                await this.addCameraBarcodeFile(
                  selectedProfileItem,
                  x,
                  false
                ).then(
  ...
  private async addBookmark(
    ...
    tempIndex.PropertyId = this.ActiveProperty.IsProxy
      ? 0
      : this.ActiveProperty.Id;
    tempIndex.ProxyPropertyId = this.ActiveProperty.IsProxy
      ? this.ActiveProperty.Id
      : 0;
    ...
    tempIndex.IsSuggest = this.IsMetattachment
      ? false
    ...
    await this.itemService
      .upsertBookmark(tempBookmark, this.ProfileItem.Id, this.UserTypes)
      .then(
        () => {
          this.IsMy = false;
  ...
  private async showSave() {
    this.uxNotifierService.showToast(
      'Saved successfully!',
      this._constants.ToastColorGood
    );
  ...
  private async addCameraBarcodeFile(
    profileItem: IProfileItemDto,
    lineitem: ILineitemDto,
    isSaveToRoom: boolean
  ) {
    ...
    if (this.pageTitle == 'Add Item' && this._type == 'barcode') {
      if (this._scanType == 'QR_CODE') {
        this.TempActiveItem.QrCode.PropertyId = this.ActiveProperty.IsProxy
          ? 0
          : this.ActiveProperty.Id;
        this.TempActiveItem.QrCode.ProxyPropertyId = this.ActiveProperty.IsProxy
          ? this.ActiveProperty.Id
          : 0;
        this.TempActiveItem.QrCode.ProfileItemId = profileItem.Id;
        this.TempActiveItem.QrCode.LineItemId = lineitem.Id;
        this.TempActiveItem.QrCode.IsMy = this.IsMetattachment
          ? false
          : this.IsMy ?? false;
        this.TempActiveItem.QrCode.IsWishlist = this.IsMetattachment
          ? false
          : this.IsWishlist ?? false;
        this.TempActiveItem.QrCode.IsSuggest = this.IsMetattachment
          ? false
          : this.IsSuggest ?? false;
        ...
        await this.barcodeService.postQrCode(this.TempActiveItem.QrCode).then(
          (x: AssetIndexDto) => {
            ...
            if (!this._isFromItemAddPage) {
              this.uxNotifierService.showToast(
                'Qr Code was saved successfully!',
                this._constants.ToastColorGood
              );
            }
          ...
          (err) => {
            ...
            this.uxNotifierService.showToast(
              'Qr Code was not saved!',
              this._constants.ToastColorBad
            );
          }
        );
      } else {
        this.TempActiveItem.Product.PropertyId = this.ActiveProperty.IsProxy
          ? 0
          : this.ActiveProperty.Id;
        this.TempActiveItem.Product.ProxyPropertyId = this.ActiveProperty
          .IsProxy
          ? this.ActiveProperty.Id
          : 0;
        this.TempActiveItem.Product.ProfileItemId = profileItem.Id;
        this.TempActiveItem.Product.LineItemId = lineitem.Id;
        this.TempActiveItem.Product.IsMy = this.IsMetattachment
          ? false
          : this.IsMy ?? false;
        this.TempActiveItem.Product.IsWishlist = this.IsMetattachment
          ? false
          : this.IsWishlist ?? false;
        this.TempActiveItem.Product.IsSuggest = this.IsMetattachment
          ? false
          : this.IsSuggest ?? false;
        ...
        await this.barcodeService
          .postBarcodeProduct(this.TempActiveItem.Product)
          .then(
            (x: AssetIndexDto) => {
              localStorage.setItem('AssetIndex', JSON.stringify(x));
              if (this._loading != undefined) {
              ...
              if (!this._isFromItemAddPage) {
                this.uxNotifierService.showToast(
                  'Upc product was saved successfully!',
                  this._constants.ToastColorGood
                );
              }
  ...
  private async saveAsync() {
    if (this._type === 'Bookmark' && this.pageTitle === 'Add Item') {
      ...
      if (this.LineItem == undefined || this.LineItem.Id == undefined) {
        ...
        localStorage.setItem(
          'TempActiveItem',
          JSON.stringify(this.TempActiveItem)
        );
        this.QueryParams = {
        ...
        if (this.pageTitle != 'Add Item') {
          switch (this.TempActiveItem.AssetInfo.Type) {
            case 'GoogleProduct':
              ...
              googleProductDto.Description =
                this.TempActiveItem.AssetInfo.Description;
              ...
              this.itemService
                .updateGoogleProduct(googleProductDto, this.UserTypes)
                .then(
                  () => {
                    if (this._loading != undefined) {
                      this._loading.dismiss();
            ...
            case 'Amazon':
              ...
              this.itemService
                .updateAmazon(amazonDto, this.ProfileItem.Id, this.UserTypes)
                .then(
                  () => {
                    if (this._loading != undefined) {
                      this._loading.dismiss();
            ...
            case 'Bookmark':
              ...
              this.itemService
                .upsertBookmark(bookmark, this.ProfileItem.Id, this.UserTypes)
                .then(
                  () => {
                    if (this._loading != undefined) {
                      this._loading.dismiss();
  ...
  public async postDigiDocToApi(
    ...
    digiDocDto.Url = url;
    digiDocDto.PropertyId = this.ActiveProperty.IsProxy
      ? 0
      : this.ActiveProperty.Id;
    digiDocDto.ProxyPropertyId = this.ActiveProperty.IsProxy
      ? this.ActiveProperty.Id
      : 0;
    digiDocDto.ProfileItemId = profileItem.Id;
    digiDocDto.LineItemId =
      lineitem == undefined || lineitem == null ? 0 : lineitem.Id;
    digiDocDto.IsMy = this.IsMetattachment ? false : this.IsMy ?? false;
    digiDocDto.IsWishlist = this.IsMetattachment
      ? false
      : this.IsWishlist ?? false;
    digiDocDto.IsSuggest = this.IsMetattachment
      ? false
      : this.IsSuggest ?? false;
    digiDocDto.IsMetattach = this.IsMetattachment;
    digiDocDto.AssetInfo = this.TempActiveItem.AssetInfo;
    ...
    if (isSaveToRoom) {
      await this.profileItemImageService
        .upsertProfileItemImageAsync(digiDocDto, this.UserTypes)
        .then(
          () => {
            if (this._loading != undefined) {
              this._loading.dismiss();
      ...
      this.itemService.upsertDigiDoc(digiDocDto, this.UserTypes).then(
        (x: AssetIndexDto) => {
          ...
          if (!this._isFromItemAddPage) {
            this.uxNotifierService.showToast(
              'DigiDoc was saved successfully!',
              this._constants.ToastColorGood
            );
          }
